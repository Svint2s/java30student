...
//				.map(this::studentToStudentResponseDto)
	...
	public long countStudentByName(String name) {
		return studentRepository.countStudentByName(name);
	}

	@Override
	public List<StudentResponseDto> findStudentsByExamScores(String examName, int minScore) {
		return studentRepository.findByExamScores("scores."+examName, minScore)
				.map(this::studentToStudentResponseDto)
				.collect(Collectors.toList());
	}
